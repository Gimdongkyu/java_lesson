...
public static void main(String[] args) {
		/*
		 * 학생 성적 분포(국어) 보고서를 만듭니다.
		 * 학생 인원은 100명 - 점수는 랜덤값으로 테스트(0<= 난수 <=100
		 * 
		 * 90~100 : 0명(소수점 1자리%)		카운트 변수가 5개 필요합니다.(cntA,cntB,cntC)
		 * 80~89 : 0명(소수점 1자리%)					ㄴcounts[5] : counts[0],counts[0],counts[1]
		 * 70~79 : 0명(소수점 1자리%)counts[2]
		 * 60~69 : 0명(소수점 1자리%)counts[3]
		 * 60점 미만 : 0명(소수점 1자리%)counts[4]
		 * 
		 */
	int[] koreans = new int[5];
...
for(int i = 0; i<counts.length;i++)	
	...
	//배열에서 기능이 향상(데이터삭제/삽입 크기도 동적으로 변경)된것이 자바 ArraysList
