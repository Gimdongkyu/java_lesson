...
//		String 클래스의 split(분열시키다,쪼개다.) 메소드 : 리턴타입이 String[]
	...
	// limit은 리턴되는 배열의크기. 단 쪼개지는 갯수보다 커질수는 없음
	System.out.println("result length: " + result.length);
	for(String t : result)
		System.out.println("result 요소 : " + t);
		...
		//Array.asList 는 테스용으로  또는 간단한 고정 리스트 사용으로 만들때 유용합니다.
		List<Integer> ilst = Arrays.asList(1,2,6,9,100);
