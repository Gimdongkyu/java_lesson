...
public static void main(String[] args) {
	...
	System.out.println("초기 result : " + result); // result = "test";  result = new String("test"); 
	System.out.println("1. String 타입 테스트");
	for(int i = 2; i<6;i++) {
		// 문자열 내용이 바뀌면 문자열 객체 새로생성되고 위치가 달라집니다.
		result += i +"/";	// result = result + (i + "/");
		System.out.println("result : " + result);
	
	}
	
	StringBuffer sb = new StringBuffer(); // StringBuilder와 함꼐 많이 사용되는 클래스
	System.out.println("2. StringBuffer 타입 테스트");
	for(int i =2; i<6;i++) {
// StringBuffer 객체이 문자열 내용이 바뀌어도 기존 객채에 추가되는 동작을 합니다.
// 반복되는 연결 연산이 많은 경우 String?X StringBuffer?O 효율적입니다.		
	...
	System.out.println("3. cintains메소드 확인");
	System.out.println("contains(result)"+sample.contains(result)); //true
	System.out.println("contains(sb)"+sample.contains(sb)); //false

	// 메소드의 형식 또는 다른 여러 형식들을 확일할 때 : 1) 클래스 or 인터페이스 2) 인터페이스일때 구현체는 무엇인가?

	// 참고 : ArrayList 선언할때 변수 선언 List 인터페이스 타입으로 합니다.
	// 		 HashMap    선언할때 변수 선언 Map 인터페이스 타입으러 합니다.	
