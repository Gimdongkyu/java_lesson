...
public class AnimalTest2 {
	...
	public static void main(String[] args) {
		...
		Dog puddle = new Dog();
		Crow crow = new Crow();
	 // 자식객체가 생성될때 먼저 부모클래스 객체ㅐ를 생성하면서 부모클래스 생성자 실행
		Dog dog = new Dog();
	...
	// 다형성
	//구현 예시 1 : 메소드 인자(또는 리턴) 타입이 Animal : Animal,Dog, Crow, Frog, Cat
public static void animal(Animal animal) {
	//구현 예시2 : 메소드 재정의. animal 객체 인스턴스 타입에 따라서 실행되는 act가 다르다. 
	animal.act();
	
	animal.print();
	// 인자로 전달받은 참조값 animal 객체가 참조하는 것의 진짜 타입(인스턴스 타입) 검사 : instanceof
...
/*
Animal 부모타입 참조는 자식클래스 필드와 메소드 접근 못 함.
	animal.jump(); // Frog
	animal.run();  //  Dog
	animal.fly();  //  Crow	
	animal.sound();//   Cat	
...
class Frog extends Animal { 
	...
	//다형성: 메소드 오버라이드  *비교: 오버로딩(overloading) - 각자 찾아보기*
	//		ㄴ 부모 클래스의 메소드를 재정의. 이때 인자와 
